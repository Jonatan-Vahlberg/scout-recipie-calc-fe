...
type ContextInterface = {
  ...
  isOpen: boolean;
  addToCart: (item: CartItem, onSuccess?: VoidFunction, onError?: VoidFunction) => void;
...
const CartProvider: React.FC = ({children}) => {
  ...
  const addToCloudCart = (item: CartItem, onSuccess?: VoidFunction, onError?: VoidFunction) => {
    setUpdating(true)
    userKit.updateUserCart(user.cart?.id, item)
    .then(res => {
      setCloudCart(res.data)
      onSuccess && onSuccess()
    })
  ...
  const addToLocalCart = (item: CartItem, onSuccess?: VoidFunction, onError?: VoidFunction) => {
    setCart([
      ...cart,
      item
    ])
  ...
  const addToCart = (item: CartItem, onSuccess: VoidFunction = () => {}, onError: VoidFunction = () => {}) => {
  ...
  const removeFromCloudCart = (item: CartItem, onSuccess: VoidFunction = () => {}, onError: VoidFunction = () => {}) => {
    setUpdating(true)

    let tempCart = {...cloudCart}
    tempCart.items = tempCart.items.filter(i => i.id !== item.id)

    userKit.updateUserCart(user.cart?.id, tempCart)
    .then(res => {
      setCloudCart(res.data)
      onSuccess && onSuccess()
    })
  ...
  const removeFromLocalCart = (item: CartItem, onSuccess: VoidFunction = () => {}) => {
    setCart(state => state.filter(i => {
      if(i.id && item.id){
        return i.id !== item.id
      }
